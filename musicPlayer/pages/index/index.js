...
Page({
  ...
  onLoad: async function (options) {
   // 请求轮播图数据,不使用下面的方法，使用封装的request方法
   /* wx.request({
      url: this.baseURI + "/banner",
      data: {type: 2},
      success: (res) => {
        console.log("请求成功",res)
      },
      fail: (res) => {
        console.log("失败成功",res)
      }
    })*/
    //使用封装的request来发送请求,要打开微信开发工具的增强编译
    //获取轮播图数据
    ...
    this.setData({
      recommendData: recommendData.result,
    })
    ...
    * */
    ...
    // 更新topList的状态值, 放在此处更新会导致发送请求的过程中页面长时间白屏，用户体验差，要等5次请求发完后才有数据
    // this.setData({
    //   topList: resultArr
    // })
